...
exports.Population = function(name) {
    ...
    this.respawn_sample= function (sample_id, callback) {
        var self = this;
        redis.smembers(sample_id,function(err,results){
            redis.pipeline()
                .sadd(self.name,results)
                .del(sample_id)
                .lrem(self.sample_queue,0,sample_id)
                .exec(function(err,results){
            ...
            console.log("[DEBUG][respawn_sample sample_id] %s", JSON.stringify(results));
        });
...
pop.respawn_sample('pop:sample:10',function(err, result) {
        console.log(result);
    }
